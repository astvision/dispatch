...
- Only members listed in a group of the instance config, for the projects of their groups, in their own private chat with the bot (or, to start a task, in a group linked to their project), and only through messages, commands, buttons and replies to plans. Everyone else is refused or ignored.
...
- Only whoever may already manage the config — an admin in a team, the one member of a personal instance — can link a group to a project or unlink one; anyone else who adds the bot to a group, or commands it in an unknown group, makes it leave (ADR 0023). A linked group sees exactly what any group sees: one-line headlines and outcomes for its projects' tasks, and `/status@bot` for them — never a plan, the agent's actions or a task's cost. A member of a linked group may start a task there with `/task@bot` or by mentioning the bot, but the task's text, plan and result stay in their private chat; the group gets only a one-line confirmation, or a hint to press Start. A member mentioned there by another member of the group's projects gets the message as a task in their own private chat, with the author's first name as its last line (G-1c). To see such mentions the bot must be the group's admin, and then Telegram sends it every message in the group; Dispatch acts only on those mentioning a member or the bot, and of the rest keeps only a member sender's username. Dispatch stores members' Telegram usernames (the `telegram_user` table), and only members', so an `@username` resolves to a member.
...
- Claude Code's permission modes and deny rules are guardrails, not a security boundary (ADR 0009). The same holds for Codex's read-only sandbox and Gemini CLI's approval modes used for planning (ADR 0026).
...
- The assistant that answers a member's plain private messages (ADR 0024, personal instances only) reads that member's project clones and, through `dispatch ask`, their tasks under the rules above; it runs no other command and writes nothing (`dontAsk` mode, the tools `Read,Grep,Glob,Bash,Skill`, only `Bash(dispatch ask *)` allowed). The member it answers for is set by a script Dispatch writes per member, not by the model. Everything it would change is a button the member taps, checked again when tapped; a repository file that steers it can at most make it propose something the member then sees.
...
- Execution runs in auto mode: the agent edits files and runs builds and tests as the instance user, within what Claude's classifier allows (ADR 0009). A project on Codex executes with `sandbox_mode="danger-full-access"` and on Gemini CLI with `--approval-mode yolo` (ADR 0026): the same user, without Claude's classifier, and only told — not prevented — not to commit, push or run `gh`. Dispatch's secrets are withheld from every agent's environment.
