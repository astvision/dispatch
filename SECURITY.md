...
**Boundaries**
- The hard boundary is the instance's OS user. Give it access to its own team's repositories only, and a GitHub token scoped to them (ADR 0005).
...
- **Redaction:**
  - What is masked: log lines, stack traces and every Telegram message Dispatch sends.
...
- **Local secret files:** planning runs never get `copyFiles` such as `.env`, so local secrets cannot end up quoted in a plan posted to the group.
