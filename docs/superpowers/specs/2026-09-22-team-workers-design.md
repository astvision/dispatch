...
1. `/api/worker/next`: a long poll of up to 25 s. Answers `{}` or a job: task number, run number and kind; project name, base branch and repo; model, effort, timeout and budget; session id and whether to resume; the prompt inputs; the attachments to fetch.
