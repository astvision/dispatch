...
- Task 8 (user-agreed, from the live run): **the chat menu button is not the Mini App.** The spec said to make each member's menu button a Web App button, but Telegram's menu button is either the command list or a web app, never both — setting it took the commands away, which the user noticed at once and rejected. `/manage` is now the only way in, and each start puts the menu button back to `commands` (`BotApi.setCommandsMenuButton`), which also repairs chats an earlier build changed. Files: `src/main/java/dispatch/App.java`, `src/main/java/dispatch/telegram/BotApi.java`, `src/test/java/dispatch/AppTest.java`. The spec's "Opening it" section and ADR 0019 need the same correction (see below).
