...
- ~~A member's private chat id equals their user id, as the menu button is set with it in Task 8.~~ Moot: Dispatch no longer touches the chat menu button, so nothing depends on this.

## Deviations

- Verification note: seven files named in `Files:` blocks are absent from the final diff, all deliberately. `ManageApi.java`, `SetupApi.java`, `ManageApiTest.java` and `SetupApiTest.java` were spared by the Task 2 adapter (see below). `StatusAndHistoryTest.java`, `UiCommandTest.java` and `ui/src/App.test.tsx` were listed as the regression nets for refactors that had to leave behaviour alone — they pass unchanged, which is the result those entries were asking for.

- Task 8 (user-agreed, from the live run): **the chat menu button is not the Mini App.** The spec said to make each member's menu button a Web App button, but Telegram's menu button is either the command list or a web app, never both — setting it took the commands away, which the user noticed at once and rejected. `/manage` is now the only way in. An intermediate build reset each member's menu button to `commands` at start to repair chats it had changed; **that was removed during verification too** — it would have cost every instance a Telegram call per member at every start to undo something no released build ever did, and it broke Goal Truth 2 ("no menu button is set"). The build carrying the reset had already repaired the one chat that needed it. Dispatch now never calls `setChatMenuButton`. Files: `src/main/java/dispatch/App.java`, `src/main/java/dispatch/telegram/BotApi.java`, `src/test/java/dispatch/AppTest.java`, plus the matching correction in ADR 0019 and the spec's "Opening it" section.
- Tasks 6 and 7 (user-agreed, from the live run): the phone layout was rebuilt after the user called it awful, and three real defects were found by rendering it at 390px rather than by reading the code. (1) An admin landed on **Overview**, not My tasks, because `MANAGE_PAGES` contains `/` and Telegram opens the Mini App at `/`; the landing page is now named rather than matched. (2) Seven menu items in a horizontal bar showed two and a half; the two task lists are now a `Segmented` and the management pages sit behind one icon button. (3) The Mini App shell replaced antd's `Layout` with a plain `div`, leaving `body` with **no background at all**, so antd's dark text sat on whatever the webview painted — the shell now paints `colorBgContainer` on itself and on `body`. Also: feeding the antd algorithm Telegram's `colorBgBase`/`colorTextBase` collapsed its text scale and made secondary text invisible, so only the accent colours are taken from Telegram now; measured afterwards at 4.52:1 for secondary text and 6.22:1 for links, both over WCAG AA. `ui/src/mini/TasksPage.tsx` dropped antd's deprecated `List`.
