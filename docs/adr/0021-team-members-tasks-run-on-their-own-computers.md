...
- Revoking a key stops that computer from taking or reporting any further work, and stops the run it was carrying too: the next progress report, within one `WorkerLoop` progress interval (about 10 s), tells the worker its key is dead, and it kills the agent (SIGTERM, a grace period, then SIGKILL) rather than let it run to an outcome nobody can collect. The one exception is a delivery already under way: `JobRunner` checks for a stop right after the agent finishes, but not again once it has moved into delivery, so its push and draft pull request still land. Either way it does not reach back into what that computer already has otherwise: its clones, worktrees and Claude sessions stay where they are.
