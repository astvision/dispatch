...
- **No group chat:** a group's `chatId` is optional. A task of a group without a chat belongs to its requester's private chat. The one-line announcements, which would repeat what the requester already gets, are not sent there, and a refused private message has no group to fall back to.
