...
Dispatch can serve its pages a second way: as a Telegram Mini App, opened with `/manage`, so a member sees their tasks and an admin manages the instance from a phone, without a terminal and without an SSH tunnel. It is off unless its owner turns it on, and it is the same pages `dispatch ui` serves — the same `UiServer`, the same management routes, with Telegram's signature in place of the one-time link.
...
- **It is opened from `/manage`, and the chat menu button is left alone.** This was first built as a "Manage" Web App menu button and withdrawn during the live run: Telegram's menu button is either the command list or a web app, never both, so it took `/task`, `/status` and the rest away. The command list is worth more than the shortcut. Dispatch now never calls `setChatMenuButton`.
