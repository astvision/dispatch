...
**Plan questions** (G-1d): a question is `{text, options}` with up to four options of at most 40 characters (longer ones are cut by code point, as button labels are; a plain string, as older plans have it, has no options). With the plan, the requester privately gets the first question as its own `PLAN_QUESTION` message: a button per option (`q:<task>:<planSeq>:<index>:<option>`), **✍️ Өөрөөр хариулах** (`…:w`, which sends a `PLAN_ANSWER_PROMPT` asking for a forced reply) and **🤷 Та шийд** (`…:d`, "choose the most reasonable answer and note it as an assumption"). A tap, or a reply to the question or to its prompt, stores the answer in `plan_answer`, redraws the question with it and sends the next one. The last answer queues one correction listing every question with its answer, as a typed reply would. A second **✍️** while that prompt is still open sends no other prompt; the tap's answer points at the open one. Only the requester answers; an answered question of the current plan is refused as already answered, and one of a superseded plan or of a task no longer awaiting approval as stale. A tap's refusal is its callback answer; a refused reply is answered in the private chat in the same words, and a reply without text (a sticker, a photo without caption) is asked for a text answer. An answered question's redraw cuts the answer to what fits Telegram's 4096-character limit. A typed reply to the plan is still a correction, after which the remaining questions are stale. Nothing of this goes to the group.
...
- Privacy mode stays on. The bot then receives:
  ...
  So Dispatch registers a command menu for each linked group (`setMyCommands`, chat scope: `/status`, `/history`, `/stats`, `/projects`, `/help`) at startup, and for a group when it is linked or migrated; picking a command from the menu reaches it. `/cmd@otherbot` is ignored. Edited messages are ignored. The bot needs to be a group admin, or to have privacy mode disabled, only for mentions of people (G-1c), since only then does it receive every message; everything else works without it. A mention of a developer gives them a draft whoever wrote it, member or not.
- Members' private chats with the bot are served: task messages, `/task`, `/start`, `/help`, `/status`, `/history`, `/stats`, `/cancel`, buttons and replies to plans, with their own command menu (`all_private_chats` scope). Private chats of non-members are ignored and logged. Groups not in the config: `leaveChat` + WARN. A group migrated to a supergroup has its chat id rewritten in the config automatically, applied without a restart: no ERROR, no hand edit (ADR 0023). If the bot left the new supergroup within the last 10 minutes (its own add event came first, from someone who may not link it), the config is still rewritten, the ERROR `group.migrated_after_leaving` is logged, and each member of the group who may manage Dispatch is asked privately to add the bot back.
